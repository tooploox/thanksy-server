...
class FetchStatistics
  ...
  def perform(params)
    ...
    top_recivers = fetch_slack_user("thanks_recived")
    most_reacted_thanks = fetch_thanks("popularity")
    send_thanksy_to_slack(params, top_senders, top_recivers, most_reacted_thanks)
  end
  ...
  def send_thanksy_to_slack(params, top_senders, top_recivers, most_reacted_thanks)
    response = SlackResponse.new.stats(top_senders, top_recivers, most_reacted_thanks)
